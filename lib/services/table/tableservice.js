...
* @param {Function(pk, rk, propertyName)} [options.encryptionResolver]        An encryption resolver, a function provided the row information, it should return a boolean value indicating whether to encrypt the column.
* @param {object}             [options.encryptionPolicy]                      Table encryption policy and key resolver
* @param {errorOrResult}  callback                                            `error` will contain information if an error occurs;
...
TableService.prototype._performEntityOperation = function (operation, table, entityDescriptor, optionsOrCallback, callback) {
  ...
  var encryptEntityDescriptor = passthroughCallback;

  var encryptionPolicy = options.encryptionPolicy;
  var encryptionOptions = {};
  if (encryptionPolicy) {
    if (operation === TableConstants.Operations.MERGE || operation === TableConstants.Operations.INSERT_OR_MERGE) {
      throw new Error('You cannot MERGE when an encryption policy is in use');
      // TODO: SR string
    }

    encryptionOptions = {
      policy: encryptionPolicy,
      modifyEntityDescriptor: options.modifyEntityDescriptor || false,
    };
    encryptEntityDescriptor = encryption.encryptEntity.bind(null, encryptionOptions);
  }

  var processResponseCallback = function (responseObject, next) {
    var decryptEntityResult = encryptionPolicy ? encryption.decryptEntity.bind(null, encryptionOptions) : passthroughResponseBodyCallback;
    decryptEntityResult(!responseObject || responseObject.error ? null : responseObject.response, (decryptError, decryptedBody) => {
      if (decryptError) {
        return callback(decryptError, returnObject.response);
      }
      ...
      if (operation === TableConstants.Operations.DELETE) {
        ...
        if (!responseObject.error) {
          responseObject.entityResponse = entityResult.parseEntity(decryptedBody, responseObject.headers, options.autoResolveProperties, options.propertyResolver, options.entityResolver);
        }
  ...
  encryptEntityDescriptor(entityDescriptor, (processError, descriptor) => {
    if (processError) {
      return callback(processError);
    }

    var webResource = RequestHandler.constructEntityWebResource(operation, table, descriptor, options);
    this.performRequest(webResource, webResource.body, options, processResponseCallback);
  });
};
...
function passthroughCallback(entityDescriptor, callback) {
  return callback(null, entityDescriptor);
};

function passthroughResponseBodyCallback(response, callback) {
